...
Status PiecewiseJerkSpeedNonlinearOptimizer::SetUpStatesAndBounds(
  ...
  if (FLAGS_use_soft_bound_in_nonlinear_speed_opt) {
    ADEBUG <<"mais true use soft bound";
    s_bounds_.clear();
    ...
    for (int i = 0; i < num_of_knots_; ++i) {
      ...
      for (const STBoundary* boundary : st_graph_data.st_boundaries()) {
        ...
        if (!boundary->GetUnblockSRange(curr_t, &s_upper, &s_lower)) {
          ADEBUG << "mais continue because of false get unblocksrange";
          continue;
        }
        SpeedPoint sp;
        switch (boundary->boundary_type()) {
          case STBoundary::BoundaryType::STOP:
            ADEBUG << "mais boundary type stop";
          case STBoundary::BoundaryType::YIELD:
	    ADEBUG << "mais boundary type in nonlinear optimizer is yield." ;
            s_upper_bound = std::fmin(s_upper_bound, s_upper);
            s_soft_upper_bound = std::fmin(s_soft_upper_bound, s_upper);
            break;
          case STBoundary::BoundaryType::FOLLOW:
            ADEBUG << "mais boundary type in nonlinear optimizer is follow." ;
            s_upper_bound =
          ...
          case STBoundary::BoundaryType::OVERTAKE:
            ADEBUG << "mais boundary type in nonlinear optimizer is overtake." ;
            s_lower_bound = std::fmax(s_lower_bound, s_lower);
            s_soft_lower_bound = std::fmax(s_soft_lower_bound, s_lower + 10.0);
            break;
          default:
            ADEBUG <<"mais default case and break!!!";
            break;
        }
        ADEBUG << "mais finished swithc boundary type";
      }
    ...
    ADEBUG <<"mais didn't enter soft bounds";
    s_bounds_.clear();
    // TODO(Jinyun): move to confs
    for (int i = 0; i < num_of_knots_; ++i) {
      double curr_t = i * delta_t_;
      ...
      for (const STBoundary* boundary : st_graph_data.st_boundaries()) {
        ...
        double s_upper = 0.0;
        if (!boundary->GetUnblockSRange(curr_t, &s_upper, &s_lower)) {
          ADEBUG <<"mais false get unblock s range";
          continue;
        }
        SpeedPoint sp;
        switch (boundary->boundary_type()) {
          ...
          case STBoundary::BoundaryType::YIELD:
            s_upper_bound = std::fmin(s_upper_bound, s_upper);
            ADEBUG <<"mais boundary type yield , upper bound " <<s_upper_bound;
            break;
          case STBoundary::BoundaryType::FOLLOW:
            s_upper_bound = std::fmin(s_upper_bound, s_upper - 8.0);
            ADEBUG <<"mais boundary type follow , upper bound " <<s_upper_bound;
            break;
          case STBoundary::BoundaryType::OVERTAKE:
            s_lower_bound = std::fmax(s_lower_bound, s_lower);
            ADEBUG <<"mais boundary type overtake , lower bound " <<s_upper_bound;
            break;
          default:
            ADEBUG <<"mais entered default";
            break;
        }
      }
      if (s_lower_bound > s_upper_bound) {
