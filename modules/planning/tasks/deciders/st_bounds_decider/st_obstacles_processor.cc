...
Status STObstaclesProcessor::MapObstaclesToSTBoundaries(
    PathDecision* const path_decision,bool is_in_path_lane_borrow_scenario,std::string overtake_obstacle_id) {
  // Sanity checks.
  ...
  //added by Mais
  AWARN <<"is_in_path_lane_borrow_scenario: " << is_in_path_lane_borrow_scenario;
  ...
  for (const auto* obs_item_ptr : path_decision->obstacles().Items()) {
    ...
    if (!ComputeObstacleSTBoundary(*obs_ptr, &lower_points, &upper_points,
                                   &is_caution_obstacle, &obs_caution_end_t,is_in_path_lane_borrow_scenario, overtake_obstacle_id)) {
...
bool STObstaclesProcessor::GetSBoundsFromDecisions(
  ...
  for (auto it : obs_id_to_decision_) {
    ...
    obs_st_boundary.GetBoundarySRange(t, &obs_s_max, &obs_s_min);
    if (obs_decision.has_yield() || obs_decision.has_stop()) {
      ADEBUG <<" mais entered has_yield let's check if has_overtake";
      if (it.second.has_overtake()){
        ADEBUG <<"mais has_overtake";
      }
  ...
  for (auto obs_t_edge : new_t_edges) {
    ...
    if (std::get<0>(obs_t_edge) == 1) {
      if (std::get<2>(obs_t_edge) >= s_max) {     
        AWARN << "  Apparently, it should be yielded.";
        ...
        AWARN << "  Apparently, it should be overtaken.";
        obs_id_to_decision_[std::get<4>(obs_t_edge)] =
            DetermineObstacleDecision(std::get<2>(obs_t_edge),
        ...
        AWARN << "  It should be further analyzed.";
  ...
  for (auto s_gap : s_gaps) {
   // AWARN <<"gap first: "<<s_gap.first <<"gap second: "<<s_gap.second;
...
bool STObstaclesProcessor::ComputeObstacleSTBoundary(
    ...
    double* const obs_caution_end_t,bool is_in_path_lane_borrow_scenario,std::string overtake_obstacle_id) {
  lower_points->clear();
  upper_points->clear();
  *is_caution_obstacle = false;
  ...
  //AWARN << "mais is overtaken?  " << obstacle.IsOvertaken();
  const auto obstacle_sl = obstacle.PerceptionSLBoundary();
  double obstacle_s = (obstacle_sl.start_s()+obstacle_sl.end_s())/2;
  //AWARN << "Mais obstacle id is: "<<obstacle.Id()<<"is blocking obstacle? "<<obstacle.IsBlockingObstacle()<<" ,,, obstacle_s is "<< obstacle_s;
  //obstacle.SetOvertaken();
 // if ((obs_trajectory.trajectory_point().empty()||obstacle_s<66)&&(overtake_obstacle_id==obstacle.Id())) {
  if (obs_trajectory.trajectory_point().empty()||obstacle_s<66) {
    ...
    *obs_caution_end_t = planning_time_;
    ...
    for (const auto& obs_traj_pt : obs_trajectory.trajectory_point()) {
      ...
      if (GetOverlappingS(adc_path_points, obs_box, kADCSafetyLBuffer,
                          &overlapping_s)) {
        ADEBUG << "Obstacle instance is overlapping with ADC path. from s1= "<<overlapping_s.first<<"at time1= "<<obs_traj_pt.relative_time()<<"to s2= "<<overlapping_s.second<<"at the same time.";
