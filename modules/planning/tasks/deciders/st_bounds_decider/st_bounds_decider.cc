...
void STBoundsDecider::InitSTBoundsDecider(
  ...
  bool is_in_path_lane_borrow_scenario = mutable_path_decider_status->is_in_path_lane_borrow_scenario();  
  ...
  AWARN << "Maiss front static obstacle id: " << overtake_obstacle_id; st_obstacles_processor_.MapObstaclesToSTBoundaries(path_decision,is_in_path_lane_borrow_scenario,overtake_obstacle_id);
...
Status STBoundsDecider::GenerateRegularSTBound(STBound* const st_bound,
  ...
  for (size_t i = 0; i < st_bound->size(); ++i) {
    ...
    if (!available_choices.empty()) {
      ADEBUG << "mais One decision needs to be made among "
