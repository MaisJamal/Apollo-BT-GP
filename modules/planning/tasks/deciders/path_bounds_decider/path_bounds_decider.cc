...
std::vector<ObstacleEdge> PathBoundsDecider::SortObstaclesForSweepLine(
  ...
  for (const auto* obstacle : indexed_obstacles.Items()) {
    ...
    //&& obstacle->Id() == mutable_path_decider_status->front_static_obstacle_id()){
    /*
    
    
    
    if (obstacle->Id() == mutable_path_decider_status->front_static_obstacle_id()){
    	AINFO << "equal";
    	if (!mutable_path_decider_status->is_in_path_lane_borrow_scenario()){ // if the vehicle has changed the lane and is returning to the old path
    		t_approx =0;
    		AINFO << "the vehicle is returning to the old path";
    	}
    	else{
    		t_approx =0.1;
    	}
