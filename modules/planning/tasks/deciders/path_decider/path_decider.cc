...
bool PathDecider::MakeStaticObstacleDecision(
  ...
  for (const auto *obstacle : path_decision->obstacles().Items()) {
    ...
    AWARN << "obstacle_id[<< " << obstacle_id << "] type["
           << obstacle_type_name << "]";
   ...
   // if (!obstacle->IsStatic() || obstacle->IsVirtual()) {
    ...
    if (obstacle->IsStatic() && obstacle->Id() == blocking_obstacle_id &&
      ...
      // Add stop decision
      AWARN << "Blocking obstacle = " << blocking_obstacle_id<<" Longitudinal PathDecider/blocking_obstacle";
    ...
    if (sl_boundary.end_s() < frenet_path.front().s() ||
      ...
      AWARN << " Longitudinal PathDecider/not-in-s";
    ...
    if (curr_l - lateral_radius > sl_boundary.end_l() ||
      ...
      AWARN << " Lateral PathDecider/not-in-l";
      ...
      if(obstacle->IsStatic()){
        ...
        *object_decision.mutable_stop() = GenerateObjectStopDecision(*obstacle);
        ...
        if (obstacle->IsStatic() && path_decision->MergeWithMainStop(
                object_decision.stop(), obstacle->Id(),
                reference_line_info_->reference_line(),
                reference_line_info_->AdcSlBoundary())) {
          AWARN << " Longitudinal PathDecider/nearest-stop";
          path_decision->AddLongitudinalDecision("PathDecider/nearest-stop",
                                                 obstacle->Id(), object_decision);
        } else {
          ObjectDecisionType object_decision;
          object_decision.mutable_ignore();
          AWARN << " Longitudinal PathDecider/not-nearest-stop";
      ...
      if (sl_boundary.end_l() < curr_l - min_nudge_l) {  // &&
        ...
        path_decision->AddLateralDecision("PathDecider/left-nudge",
                                          obstacle->Id(), object_decision);
        AWARN << " Lateral PathDecider/left-nudge";
        ...
        path_decision->AddLateralDecision("PathDecider/right-nudge",
                                          obstacle->Id(), object_decision);
        AWARN << " Lateral PathDecider/right-nudge";
      }
    }
  }
