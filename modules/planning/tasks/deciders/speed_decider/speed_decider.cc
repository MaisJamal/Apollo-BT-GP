    ...
    switch (location) {
      case BELOW:
        if (boundary.boundary_type() == STBoundary::BoundaryType::KEEP_CLEAR) {
          ...
          if (IsFollowTooClose(*mutable_obstacle)) {
            ...
            AWARN <<"true IsFollowTooClose";
          } else {  // high speed or low speed accelerating
            AWARN <<"false IsFollowTooClose. high speed or low speed accelerating";
            // FOLLOW decision
          ...
          AWARN <<"false CheckIsFollow, Yield decision";
          // YIELD decision
      ...
      case ABOVE:
        if (boundary.boundary_type() == STBoundary::BoundaryType::KEEP_CLEAR) {
          ...
          AWARN <<"in ABOVE case, overtake decision";
          // OVERTAKE decision
...
bool SpeedDecider::CheckIsFollow(const Obstacle& obstacle,
  ...
  if (obstacle_l_distance > FLAGS_follow_min_obs_lateral_distance) {
    AWARN <<"false chechIsFollow due to obstacle_l_distance: "<<obstacle_l_distance <<" > " <<"FLAGS_follow_min_obs_lateral_distance:"<< FLAGS_follow_min_obs_lateral_distance;
    return false;
  }

  // move towards adc
  if (boundary.bottom_left_point().s() > boundary.bottom_right_point().s()) {
  AWARN <<"false chechIsFollow due to boundary.bottom_left_point().s(): " <<boundary.bottom_left_point().s() <<" > "<< "boundary.bottom_right_point().s(): "<<boundary.bottom_right_point().s();
    return false;
  }
  ...
  if (boundary.min_t() > kFollowCutOffTime ||
    ...
    AWARN <<"false chechIsFollow due to boundary:boundary.min_t(): "<<boundary.min_t()<<" boundary.max_t(): "<< boundary.max_t();
    return true;// false;
  }

  // cross lane but be moving to different direction
  if (boundary.max_t() - boundary.min_t() < 0.3){//FLAGS_follow_min_time_sec) {
    AWARN <<"false chechIsFollow due to boundary.max_t(): "<<boundary.max_t() <<" boundary.min_t(): "<<boundary.min_t() << "FLAGS_follow_min_time_sec " <<FLAGS_follow_min_time_sec; 
    return false;
  }
