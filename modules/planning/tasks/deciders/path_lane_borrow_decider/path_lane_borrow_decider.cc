...
Status PathLaneBorrowDecider::Process(
  ...
  if ((FLAGS_enable_skip_path_tasks && reference_line_info->path_reusable() ) || reference_line_info->Keep_lane()) {
    // for debug
...
bool PathLaneBorrowDecider::IsNecessaryToBorrowLane(
  ...
  //        blocking_obstacle_id);
   //   bool* overtake ;
    //  *overtake = true;
  //    blocking_obstacle->SetOvertaken(*overtake);
    ...
    if (path_decider_status.decided_side_pass_direction().empty()) {
      ...
      if (!left_borrowable && !right_borrowable) {
        ...
        if (left_borrowable&&reference_line_info.Switch_to_left()) {
          mutable_path_decider_status->add_decided_side_pass_direction(
              PathDeciderStatus::LEFT_BORROW);
        }
        if (right_borrowable&&reference_line_info.Switch_to_right()) {
          mutable_path_decider_status->add_decided_side_pass_direction(
