      ...
      case BELOW:
        if (boundary.boundary_type() == STBoundary::BoundaryType::KEEP_CLEAR) {
          ...
          if (mutable_path_decider_status->is_in_path_lane_borrow_scenario()){
    		AWARN << "It's considered as OVERTAKE";
               ...
               mutable_obstacle->AddLongitudinalDecision("dp_st_graph/overtake",
                                                         overtake_decision);
               AWARN << "overtake decision is taken";
               }
    	  } else {
    	    // YIELD decision
...
bool MiptSpeedDecider::CreateYieldDecision(
  ...
  if (main_stop_s < reference_line_fence_s) {
    AWARN << "Yield reference_s is further away, ignore.";
    return false;
  }
  AWARN <<"the yield distance is: "<< yield_distance_s;
  ...
  AWARN << "YIELD: obstacle_id[" << obstacle.Id() << "] obstacle_type["
         << PerceptionObstacle_Type_Name(obstacle_type) << "]";
...
bool MiptSpeedDecider::CreateOvertakeDecision(
  ...
  const double main_stop_s =
      reference_line_info_->path_decision()->stop_reference_line_s();
  if (main_stop_s < reference_line_fence_s) {
    AWARN << "Overtake reference_s is further away, ignore.";
    return false;
  }
  AWARN <<"the overtake distance is: "<< overtake_distance_s;
  auto ref_point = reference_line_->GetReferencePoint(reference_line_fence_s);
  ...
  PerceptionObstacle::Type obstacle_type = obstacle.Perception().type();
  AWARN << "OVERTAKE: obstacle_id[" << obstacle.Id() << "] obstacle_type["
         << PerceptionObstacle_Type_Name(obstacle_type) << "]";
